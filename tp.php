...
<body>
        ...
        <div id="left-buttons-container">
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-content">
                    <a href="demo.php" id="newFile">New File</a>
                    <a href="#" id="saveFile">Save File</a>
					
                    <a href="demo.php" id="downloadFile">Download File</a>
                    <a href="demo.php" id="deleteFile">Delete File</a>
                </div>
            </div>
        </div>
    ...
    // Prompt the user to input the length between the nodes
    const length = prompt("Enter the length between the nodes in meters:");

    // Check if the input is a valid number
    if (length === null || isNaN(parseFloat(length))) {
        alert("Please enter a valid length.");
        return; // Exit function if input is invalid
    }

    // Convert the input length to a number
    const lengthInMeters = parseFloat(length);

    // Continue with the connection if the length is valid
...
function generateSequentialId() {
    return nodeIdCounter++;
        ...
        // Function to draw the connection between nodes
function drawConnection(node1, node2) {
    // Calculate the midpoint of the line
    const midX = (node1.x + node2.x) / 2;
    const midY = (node1.y + node2.y) / 2;

    // Calculate the length between the nodes
    const length = Math.sqrt(Math.pow(node2.x - node1.x, 2) + Math.pow(node2.y - node1.y, 2));
    const lengthInMeters = length / 10; // Assuming 1 unit in canvas = 1 meter, adjust accordingly
    ...
    // Draw the length text above the line
    context.fillStyle = '#000'; // Set color to black
    context.font = '12px Arial'; // Set font style
    context.fillText(lengthInMeters.toFixed(2) + ' meters', midX, midY - 5); // Display length above the line
}
